...
const collections = [
...

  {
    ...
    fields: [
      ...
      { key: "healPower.min", label: "Base Heal Power (min, 0 = non-healer)", type: "number" },
      { key: "healPower.max", label: "Base Heal Power (max, 0 = non-healer)", type: "number" },
      { key: "critChance.min", label: "Base Crit Chance % (min)", type: "number" },
      { key: "critChance.max", label: "Base Crit Chance % (max)", type: "number" },
      { key: "critDamage.min", label: "Base Crit Damage bonus % (min, e.g. 120 = 2.2x)", type: "number" },
      { key: "critDamage.max", label: "Base Crit Damage bonus % (max)", type: "number" },
      ...
      { key: "basicAttack.sound", label: "Basic attack sound (blank = element auto)", type: "sound" },
      { key: "passives", label: "Passives (JSON list, e.g. [{kind:'damageBonus',mult:0.1}]) — use GameCraft editor for guided editing", type: "string" },
      ...
      { key: "growth.healPower", label: "Heal Power per level (0 = non-healer)", type: "number" },
      { key: "growth.critChance", label: "Crit Chance % per level", type: "number" },
      { key: "growth.critDamage", label: "Crit Damage bonus % per level", type: "number" },
      {
        key: "evolveBonus",
...

  {
    ...
    fields: [
      ...
      { key: "tags", label: "Species tags (comma-separated, e.g. beast, undead)", type: "array" },
...

  {
    ...
    fields: [
      ...
      { key: "cooldown", label: "Cooldown (usable every N rounds, blank = every round)", type: "number" },
      { key: "duration", label: "Buff duration in rounds", type: "number" },
      { key: "buffs", label: "Buffs (e.g. [{kind:'healblock',value:0.5}] — kinds: attack, defense, regen, weaken, expose, dot, wet, frozen, shield, magicBoost, healblock)", type: "string" },
      { key: "power", label: "Damage (1.6 = 1.6× scaling stat)", type: "number" },
      { key: "baseDamage", label: "Base damage (flat, added on top — 0 = none)", type: "number" },
      { key: "scaling", label: "Scales from (blank = auto: physical->attack, else magic)", type: "choice", options: ["attack", "magicPower", "healPower"], allowBlank: true },
      ...
      { key: "bonusVsHighHp", label: "Bonus vs high HP (e.g. {aboveHpPct:0.7,mult:0.5})", type: "string" },
      { key: "bonusVsTags", label: "Bonus vs tags (e.g. {tags:['golem'],mult:0.5})", type: "string" },
      { key: "execute", label: "Execute (e.g. {belowHpPct:0.3} gate or {finishBelowHpPct:0.1} kill)", type: "string" },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "name", label: "Display Name", type: "string" },
      { key: "when", label: "Target Status", type: "choice", options: ["wet", "frozen", "dot", "expose", "weaken", "poison", "healblock"] },
...
const pages = [
  ...
  },
  {
    ...
    fields: [
      ...
      { path: "town.rest.stamina", label: "Rest stamina gain", type: "number" },
      { path: "town.rest.maxPerDay", label: "Rests allowed per day", type: "number" },
      { path: "town.temple.stamina", label: "Temple stamina cost", type: "number" },
      { path: "town.tavern.sleep.price", label: "Tavern sleep price (gold)", type: "number" },
      { path: "town.sleepOutside.lifeLossChance", label: "Sleep outside heart-loss chance (0-1)", type: "number" },
      { path: "temple.origin.item", label: "Rebirth stone item id", type: "string" },
      { path: "temple.origin.price", label: "Rebirth stone price (gold)", type: "number" },
      { path: "temple.origin.startQty", label: "Rebirth stones at start", type: "number" },
    ],
  },
  {
    id: "races",
    ...
    idLabel: "id",
    nameField: "name",
    itemLabelTemplate: "{name} ({chance}%)",
    fields: [
      { key: "name", label: "Name", type: "string" },
      { key: "desc", label: "Description", type: "string" },
      { key: "chance", label: "Roll weight (higher = more common)", type: "number" },
      { key: "color", label: "Color hex (e.g. #c0b283)", type: "string" },
      { key: "glow", label: "Glow hex for rare races (blank = same as color)", type: "string" },
      { key: "stats", label: "Stats (e.g. {attack:6, maxHp:30})", type: "string" },
    ],
  },
  {
    id: "loot",
